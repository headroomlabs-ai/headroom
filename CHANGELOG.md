...
* **opencode:** add OpenCode (sst/opencode) support with `headroom wrap opencode`. OpenCode is configured via `opencode.json` (it ignores `OPENAI_BASE_URL`), so the wrapper routes traffic by overriding the built-in `anthropic` and `openai` providers' `options.baseURL` to the local proxy while preserving the user's existing config and model selection. The edit is reversible with `headroom unwrap opencode` (byte-for-byte restore from a pre-wrap backup), idempotent across re-wraps/port changes, and per-project savings are attributed via a `/p/<name>` base-URL prefix. rtk guidance is injected into `AGENTS.md`.
