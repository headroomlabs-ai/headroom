...
* **install (windows):** `headroom install status` no longer terminates a live deployment. `runtime_status()` probed liveness with `os.kill(pid, 0)` and only caught `OSError`; on Windows `os.kill` runs `TerminateProcess` (so the "check" killed the proxy) and a detached-group PID raised `WinError 87` as a `SystemError` that escaped and crashed the command. It now uses a Windows-safe `_pid_alive()` helper (prefers `psutil.pid_exists`, rejects non-positive PIDs, and catches `SystemError`), mirroring `headroom.cli.wrap._pid_alive` from #1315 ([#1544](https://github.com/headroomlabs-ai/headroom/issues/1544)).
