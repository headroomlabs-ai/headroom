...
* **proxy:** never forward a `role="system"` message inside the Anthropic `messages[]` array. On large contexts a transform/pipeline step could relocate the harness system block into `messages[0]`, which Anthropic rejects with a hard `400 invalid_request_error` ("messages.0: use the top-level 'system' parameter for the initial system prompt"). The Anthropic forwarder now relocates any stray system-role message back into the top-level `system` parameter as its last step before sending upstream, so compression-triggered requests no longer fail ([#765](https://github.com/headroomlabs-ai/headroom/issues/765)).
