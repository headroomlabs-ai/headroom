...
* **proxy:** Rust-native savings stats + dashboard (`observability/stats.rs`), the Rust re-implementation of `savings_tracker.py` (realignment Phase H). The Rust proxy now serves `GET /stats` (backend-agnostic savings JSON) and `GET /dashboard` (embedded UI) by default. Because the proxy fronts every backend in one process, savings are attributed per provider/model and unified across Anthropic, OpenAI, Bedrock (invoke + streaming) and Vertex with no "combine"/"federation" step. USD is valued from a models.dev price book (`--pricebook` / `HEADROOM_PROXY_PRICEBOOK`); by default it uses a compiled-in models.dev snapshot (vendored in `data/models_dev.json`, refreshed via `scripts/refresh_model_limits.sh`) so USD valuation works out of the box with no startup network call — override with an `http(s)://` URL to fetch live prices once at startup, a local file path, or `""` to disable (fail-open to tokens-only USD-zero if an override is unreachable); stats persist across restarts (`--savings-path`); and `--upstream-stats-url` transitionally folds in blocks still produced only by the Python proxy so the Rust dashboard is the single source of truth during migration. Persistence runs off the request hot path — `record` only updates memory and marks the store dirty, while a background task and the shutdown hook perform the disk I/O — and startup warns when compression is enabled but the compression mode is off (otherwise savings silently read 0). Failed upstreams count toward request/failure totals but accrue no savings (no inflation, no double-count on retry); `requests.compressed` exposes how much traffic was actually compressed so `savings_percent` (the reduction over compressed input) is read in context. The savings store (`observability/`) is 100%-covered and gated at 100% patch coverage in CI (`cargo-llvm-cov` line coverage on stable); the proxy/recorder wiring is covered by integration tests. Migration scope: the dashboard is shared with the Python proxy. Panels the Rust proxy does not yet produce natively (prefix-cache, throughput, latency-breakdown, agent-usage, per-project savings ([#802](https://github.com/chopratejas/headroom/issues/802)), waste-signals, savings sparkline, config badge) are folded in from the still-running Python proxy via `--upstream-stats-url` — `SUPPLEMENTAL_KEYS` allowlists every such top-level block — so the dashboard stays whole during migration; in a Rust-only deployment (no `--upstream-stats-url`) those panels are empty until each block is ported to Rust. The prefix-cache USD/`tokens.output` figures read 0 until per-request cache/output token recording lands (the price fields exist; no lane populates the token counts yet). The historical per-model chart and weekly/monthly granularities are not yet emitted (`/stats-history` emits `series.daily` only). `--savings-path` writes a new Rust-native file; it does not migrate an existing Python `proxy_savings.json` — start fresh (loading the old file would under-count lifetime input tokens; `savings_percent` is anchored on recorded forwarded input and reports 0 rather than a misleading ~100% when that anchor is absent).
...
* **proxy:** fix Bedrock cross-region inference pricing — `eu.`/`us.`/`au.`/`jp.`-prefixed model IDs (e.g. `eu.anthropic.claude-haiku-4-5-20251001-v1:0`) now resolve to the Bedrock-specific price rather than falling back to the cross-provider consensus rate. The models.dev Bedrock catalog uses `global.`-prefixed IDs; the price book now also indexes the region-stripped base key so all four request prefixes match the correct Bedrock entry.
* **proxy:** fix savings history chart showing a spurious large bar on the first surviving date after raising `--max-history`. Previously, `with_path()` reset `history_evicted` to `false` when `history.len() < max_history` on the assumption that all data was present — but raising the cap does not restore previously evicted checkpoints, so the baseline was still lost. The reset no longer fires; `history_evicted` stays `true` once set.
