...
* **wrap:** add `headroom wrap grok` and `headroom unwrap grok` for first-class Grok Build CLI support via Grok's documented `~/.grok/config.toml` custom-model path, with a Headroom-managed alias that launches through the shared proxy.
