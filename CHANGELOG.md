...
* **opencode:** add OpenCode (sst/opencode) support with `headroom wrap opencode`, at parity with the Claude Code / Codex wraps. OpenCode is configured via `opencode.json` (it ignores `OPENAI_BASE_URL`), so the wrapper routes traffic by overriding the built-in `anthropic` and `openai` providers' `options.baseURL` to the local proxy while preserving the user's existing config and model selection. Like Claude/Codex it starts the proxy with the `agent-90` high-savings profile, registers the `headroom_retrieve` MCP server in `opencode.json` (via a new `OpenCodeRegistrar`, so `headroom mcp install` covers OpenCode too) so compression markers are actionable, registers Serena (`ide-assistant` context) unless `--no-serena`, injects rtk guidance into `AGENTS.md`, and attributes per-project savings via a `/p/<name>` base-URL prefix. `--no-mcp` / `--no-serena` opt out. The edits are reversible with `headroom unwrap opencode` (byte-for-byte restore from a pre-wrap backup, or override + MCP strip when the backup is gone), idempotent across re-wraps/port changes.
