...
* **proxy:** Rust live-zone **AST code compression**. `ContentType::SourceCode` tool-result blocks now route to a new `CodeAwareCompressor` Rust port (`headroom-core`), a byte-for-byte reimplementation of the Python `code_compressor`: tree-sitter parsing for Python/JavaScript/TypeScript/Go/Rust/Java/C/C++, symbol-importance scoring, per-function body-budget truncation with omitted-line summaries, Python docstring handling, and a re-parse syntax-validity guard (returns the original on any invalid output). Wired into `dispatch_compressor` like the existing structural compressors; grammars are statically linked, so no model, download, or runtime dependency. Byte-parity is locked by 30 recorded fixtures across all 8 languages.
* **proxy:** Rust live-zone **ML prose compression (Kompress)**, gated behind `--enable-kompress` / `HEADROOM_PROXY_ENABLE_KOMPRESS` (default **off**). When enabled, `ContentType::PlainText` blocks route to a Rust port of the `kompress-v2-base` compressor (ONNX via `ort`, ModernBERT tokenizer). The model is resolved **cache-only** — the proxy never downloads it on a request or startup thread; a cold cache passes plain text through unchanged, exactly like the Python reference when Kompress is unavailable. Optional startup warm-up runs off the request path. Byte-parity is locked by 21 recorded fixtures.
