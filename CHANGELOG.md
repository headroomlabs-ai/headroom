...
* **cache/ccr:** the CCR SQLite store now honors `HEADROOM_WORKSPACE_DIR`. `default_db_path()` derived its fallback from a hardcoded `~/.headroom/ccr_store.db` and only consulted the per-resource `HEADROOM_CCR_SQLITE_PATH` override, so redirecting the workspace still left a `ccr_store.db` (+ WAL/SHM) in `~/.headroom/`. The fallback now routes through `headroom.paths.workspace_dir()`, so a single `HEADROOM_WORKSPACE_DIR` override relocates the CCR store with every other persistent file; `HEADROOM_CCR_SQLITE_PATH` still takes precedence ([#1558](https://github.com/headroomlabs-ai/headroom/issues/1558)).
