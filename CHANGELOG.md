...
* **proxy (anthropic):** fix `UnboundLocalError: ccr_workspace_key` that turned every `/v1/messages` request into HTTP 500 when CCR tool/system-instruction injection was disabled (e.g. running with `--no-ccr-inject-tool` and the default `ccr_inject_system_instructions=False`). `handle_anthropic_messages` assigned the workspace key only inside the inject block but referenced it in the proactive-expansion gate that follows; the assignment is now hoisted to default `None` so the gate degrades cleanly to "workspace unresolved" instead of crashing.
