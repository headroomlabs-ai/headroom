...
* **ccr:** lowercase the hash in the streaming feedback recorders too. `_record_ccr_feedback_from_response` and `_record_ccr_feedback_from_openai_sse` extract the `headroom_retrieve` hash directly (not via `parse_tool_call`, which was already fixed) and call `store.retrieve` with it. The store keys every entry by a lowercase hash, so a model that echoed the marker hash uppercase missed the store and its retrieval never reached the TOIN learning loop. Both recorders now normalise the hash to lowercase before the lookup, completing the fix started for the request path.
