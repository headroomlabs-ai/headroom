...
* **memory:** staleness-check Windows absolute-path references. `MemoryBudgetManager._is_stale` only treated an entity ref as a path when it started with `/` or `./`, so a Windows drive-letter ref (`C:\...`) was never checked — memories pointing at files that had since been deleted were never pruned on Windows and kept being injected. Path detection now also recognises a Windows *absolute* drive path (an alphabetic drive letter, a colon, then a slash/backslash: `C:\...` / `C:/...`), so an arbitrary namespaced entity label like `A:B` is not mistaken for a filesystem path.
