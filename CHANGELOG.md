...
* **wrap:** stop leaking the proxy stdio-log file handle when the readiness probe raises. `_start_proxy` closed the log file in the success, process-death, and timeout branches, but if `_check_proxy()` raised mid-loop none of those ran, leaking one fd (and pinning the log file open) per `headroom wrap`. The readiness loop is now wrapped in `try/finally` so the handle is always closed on exit ([#1555](https://github.com/headroomlabs-ai/headroom/issues/1555)).
