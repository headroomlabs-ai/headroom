...
With ``ANTHROPIC_API_KEY`` set and ``--real`` it drives the real analyzer LLM
and scores the actually-generated rule.

Usage:
    python benchmarks/rtk_loop_learn_eval.py            # deterministic
    python benchmarks/rtk_loop_learn_eval.py --real     # real LLM (needs key)
"""
...
def run_eval(*, use_real_llm: bool) -> Scorecard:
    ...
    card.add("names_command", "grep" in text and "timeouterror" in text)
    card.add(
        "prescribes_fix",
...
def main() -> int:
    ...
    parser.add_argument(
        "--real",
        action="store_true",
        help="Drive the real analyzer LLM (requires ANTHROPIC_API_KEY).",
    )
    args = parser.parse_args()

    if args.real and not os.environ.get("ANTHROPIC_API_KEY"):
        print("--real requires ANTHROPIC_API_KEY; falling back to deterministic mode.\n")
        args.real = False
