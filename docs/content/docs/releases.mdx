...
`release.yml` calls the read-only `release-build.yml` workflow for version
resolution, npm packaging, the wheel matrix, artifact collection, and smoke
imports. Every publisher waits for that complete build group to succeed;
`create-release` also waits for the publish jobs.
...
release.yml:
  build-and-smoke -> publish-pypi            --+
                 -> publish-npm             --+
                 -> publish-github-packages --+-> create-release
                 -> publish-docker          --+
```

The immutable candidate workflow calls the same build-only workflow directly.
Its entire call tree has read-only repository permissions and contains no
publishing jobs. This separation is required because GitHub validates nested
workflow permissions before evaluating job conditions.
...
### build-wheels
Builds the Python wheel matrix for Linux x86_64, Linux arm64, Apple Silicon macOS, Intel macOS, and Windows x86_64, plus one source distribution in `.github/workflows/release-build.yml`. Linux wheels are audited for glibc symbol compatibility.
...
| File | Purpose |
|------|---------|
| `.github/workflows/release.yml` | Release orchestration and publishing |
| `.github/workflows/release-build.yml` | Shared read-only build and smoke gates |
| `.github/workflows/candidate-artifact.yml` | Immutable candidate bundle and verification |
