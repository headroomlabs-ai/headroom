...
```json
{
  ...
  "parameters": {
    ...
    "query": "Optional context hint for the concrete gap you are checking. The hint is recorded for feedback and stats; retrieval still returns the full original content."
  }
}
```
...
- Retrieve when the user explicitly asks for raw, original, full, exact, or omitted content.
- Retrieve when you need to inspect the original payload for a specific follow-up the kept summary cannot answer.
...
# The LLM answers from kept rows unless it has a concrete gap.
