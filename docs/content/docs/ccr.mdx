...
`store_max_entries`). Those reasons are best-effort: the store remembers why it removed a
hash, but not why the storage backend did. A plain `Entry not found` therefore means either
the hash was never stored, or it was expired/evicted by a backend sweep or by an earlier
proxy process, which leaves no reason behind.
