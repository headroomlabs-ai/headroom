...
**1. Install the Vertex SDK.** `google-cloud-aiplatform` ships in the dedicated
`[vertex]` extra, kept out of `[proxy]` because it and its transitive tree add
roughly 175 MB and only this backend needs it:

```bash
pip install "headroom-ai[proxy,vertex]"
```

Without it the proxy **refuses to start**, naming the fix and exiting `2`:

```text
Cannot start proxy: Vertex backend selected but the Vertex SDK is missing. ...
```

This is a startup check rather than a per-request failure, because the backend
would otherwise construct cleanly and then fail on every call with an opaque
`litellm.BadRequestError: … No module named 'vertexai'`.
...
Gemini optimization handler; `publisher=anthropic` (Claude on Vertex) is
forwarded to Vertex directly, preserving your bearer token. Any client that
...
<Callout type="warn">
These routes need **neither** `--backend vertex_ai` nor the `[vertex]` extra. Do
not combine the two: with the backend flag set, Claude publisher requests are
re-routed through LiteLLM, which derives the project, region and model from its
own configuration and ignores the ones in your URL — a request that works
without the flag can return a `404` for a model you never asked for.
</Callout>

### Reading Vertex errors

When Vertex rejects a request with `401`, `403`, `404` or `429`, Headroom appends
a `[headroom] hint: …` note to `error.message`, sets an `x-headroom-hint`
response header, and logs a warning. The hint names the likely cause — expired
ADC, a partner model not yet enabled in Model Garden, or a model that is not
served in the requested location. Successful responses are never modified.
