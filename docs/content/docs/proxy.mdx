...
| `--stateless` / `HEADROOM_STATELESS` | `false` | Keep request-derived state out of the filesystem: disables memory, TOIN, savings persistence, request logs, and the on-disk [CCR retrieval store](/docs/ccr) (an external CCR backend such as `redis` is kept -- see [CCR](/docs/ccr)). The runtime log (`~/.headroom/logs/proxy-<port>.log`), the beacon lock, subscription state, and model caches are still written. |
