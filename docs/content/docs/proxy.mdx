...
| Option | Default | Description |
|--------|---------|-------------|
| `--memory` | `false` | Enable persistent user memory and provider-appropriate memory tools. When `--mode cache` is active, context injection is skipped because prefix stability takes precedence; use `--mode token` if you need memory recall. |
...
| `HEADROOM_LOG_PAYLOAD_PREVIEW` | `off` | Include up to 4 KB of the retrieved [CCR](/docs/ccr) payload in each `headroom_retrieve` line of the runtime log (`~/.headroom/logs/proxy-<port>.log`). Off by default: the payload is verbatim tool-result content and can carry source code, credentials the redactor does not recognize, or customer data; the log otherwise records byte counts only. Turn it on with `1`/`true`/`yes`/`on`. Do not share a log captured with this on. |
...
Headroom writes the runtime log (`~/.headroom/logs/proxy-<port>.log`) and, when
`--log-file` is set, the JSONL request log. Both can hold request and response
content — `--log-messages` bodies, wire-debug dumps, and CCR payload previews
when they are enabled.

- **Linux and macOS:** every log file Headroom creates is `0600`, owner-read/write
  only, and so is every rotated backup (`proxy-8000.log.1`, …). This is not
  conditional on which content switch is on. If the log path already exists as a
  symlink, Headroom refuses to log rather than write through it, and says so.
- **Windows:** **Headroom does not restrict the log file.** Read access on NTFS is
  decided by the file's ACL; `chmod` there only toggles the read-only attribute,
  and Python has no API for setting an ACL, so Headroom does not claim a
  protection it cannot establish. The log inherits the ACL of the directory it
  is written into, and the proxy logs a warning at startup saying so. Keep
  `%USERPROFILE%\.headroom` (or your chosen `--log-file` path) off shared
  volumes and out of directories with broadened permissions.

A log shipper or sidecar running as a different user or group cannot read the
runtime log on Linux/macOS. Run the collector as the same user, or copy the file
with a privileged job.
...
Both the client IP and the inbound `Host:` header must name loopback. In container environments (Docker/Podman), requests forwarded from the container host gateway are also admitted under loopback `Host:` headers. To allow arbitrary remote callers, set `HEADROOM_COMPRESS_ALLOW_REMOTE=1`. `HEADROOM_PROXY_TOKEN` still applies if set. The same policy applies to [`POST /v1/usage`](#relaying-provider-usage-post-v1usage).
</Callout>
