...
| `HEADROOM_COMPRESS_ALLOW_REMOTE` | `false` (defaults to `1` in Docker presets) | Allow non-loopback callers to reach [`POST /v1/compress`](#post-v1compress) and `POST /v1/usage`. Required to run Headroom as a gateway/sidecar and in Docker bridge deployments; without it remote callers get `404`. |
...
Both the client IP and the inbound `Host:` header must name loopback. To allow remote callers, set `HEADROOM_COMPRESS_ALLOW_REMOTE=1` (automatically defaulted in Docker deployment manifests, since bridge networking forwards host calls with non-loopback source IPs). `HEADROOM_PROXY_TOKEN` still applies if set. The same policy applies to [`POST /v1/usage`](#relaying-provider-usage-post-v1usage).
</Callout>
