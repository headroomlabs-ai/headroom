...
## Per-request upstream override (`X-Headroom-Upstream`)
...
Clients that can attach custom per-provider headers (e.g. [OpenCode](https://github.com/anomalyco/opencode), which supports `options.headers` in its provider config) point every provider's base URL at the same proxy and tag each with its real upstream:
  ...
  "provider": {
    "deepseek": {
      "options": {
        ...
        "headers": { "X-Headroom-Upstream": "https://api.deepseek.com" }
      }
    },
    "groq": {
      "options": {
        "baseURL": "http://127.0.0.1:8787/v1",
        "headers": { "X-Headroom-Upstream": "https://api.groq.com/openai" }
      }
    }
  }
}
...
One proxy, one port — every provider is compressed and forwarded to its own upstream. Auth stays on the client (the `Authorization` / `x-api-key` header passes through), so no API keys live in the proxy config.
