...
| `HEADROOM_TREE_SITTER_PRELOAD` | Comma-separated tree-sitter languages to preload during code-aware startup. Unset loads parsers lazily per thread on first use to reduce worker RSS. | -- |
