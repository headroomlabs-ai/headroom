...
| `HEADROOM_MIN_INPUT_TOKENS` | Opt in to skipping compression below this token count; `0` disables the guard. | `0` |
| `HEADROOM_DIVERSITY_LAMBDA` | Opt in to diversity-aware Kompress sentence selection with this relevance/diversity tradeoff, clamped to `0`–`1`. | unset |
| `HEADROOM_SEMANTIC_SCORE_ENABLED` | Opt in to embedding-based semantic preservation scoring for the library `compress()` API. May load the optional relevance model. | `false` |
