...
| `HEADROOM_ANTHROPIC_AUTO_CONTINUE` | Opt in to keeping an Anthropic SSE request open when a recognized usage-limit error includes a future reset, then retrying the original request once. Ordinary authentication/billing failures pass through unchanged. | `false` |
| `HEADROOM_ANTHROPIC_AUTO_CONTINUE_MAX_WAIT_SECONDS` | Maximum reset delay accepted by Anthropic auto-continue. Longer waits pass through with the original upstream status instead of tying up the connection. Range: 1–86400 seconds. | `18300` |
