...
<Tabs groupId="lang" items={['TypeScript', 'Python', 'Go']}>
...
import (
    "time"
    "github.com/headroomlabs-ai/headroom/sdk/golang"
)

// Reads HEADROOM_BASE_URL / HEADROOM_API_KEY from env automatically.
client := headroom.NewClient()

// Or configure explicitly:
explicit := headroom.NewClient(
    headroom.WithBaseURL("http://localhost:8787"),
    headroom.WithAPIKey("your-api-key"),
    headroom.WithTimeout(30*time.Second),
    headroom.WithRetries(2),
    headroom.WithFallback(true),
    headroom.WithDefaultMode(headroom.ModeOptimize),
    headroom.WithConfig(&headroom.Config{
        ModelContextLimits: map[string]int{
            "gpt-4o":      128000,
            "gpt-4o-mini": 128000,
        },
        CacheOptimizer: &headroom.CacheOptimizerConfig{Enabled: headroom.Bool(true)},
    }),
)
_ = explicit
```
...
out, err := client.ChatCompletionsCreate(ctx, map[string]any{
    ...
    Mode:               headroom.ModeAudit,
    OutputBufferTokens: 8000,
    KeepTurns:          5,
...
#### `x-headroom-base-url` is refused when an outbound proxy is in the way

Because the destination is chosen by the client, the proxy validates it by
resolving the hostname and rejecting non-public addresses — and then dials the
address it validated, rather than resolving the name a second time at connect
time. Without that second half, a hostname whose authoritative DNS is
attacker-controlled can answer the check with a public address and the
connection with `169.254.169.254` or an RFC1918 host (DNS rebinding).

An outbound HTTP or SOCKS proxy (`HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`) is
handed the target *by name* — in the request line, in `CONNECT`, in the SOCKS5
address — and resolves it itself, from its own network, after the check ran.
Headroom cannot enforce the validated address there, so a request carrying
`x-headroom-base-url` that would route through the proxy is **refused** rather
than forwarded on a verdict nothing enforces.

This affects only client-supplied upstreams. Every operator-configured provider
URL goes through the proxy exactly as before. If you need a specific endpoint
reachable via `x-headroom-base-url` through an outbound proxy, name it in
`HEADROOM_ALLOWED_BASE_URLS` — an explicit operator decision to admit that
destination by name, which also turns off the resolve-and-pin check for it.
...
For Claude Code's official VS Code extension, use the client-owned persisted
setting instead of `ANTHROPIC_MODEL`:

```bash
headroom wrap vscode-claude --1m
```

This writes the resolved model with exactly one `[1m]` suffix to the top-level
`model` setting in Claude Code's user settings and records the prior presence and
value in Headroom's reversible sidecar. Running the command without `--1m`
restores a model previously managed by Headroom; `headroom unwrap vscode-claude`
restores the model and proxy environment values together. Use
`--no-configure --1m` to print the setting without writing either file.

Claude Code and Anthropic own model support and account eligibility. Local tests
prove settings persistence and restoration, not a live entitled VS Code session
or a resulting 1M context window.
