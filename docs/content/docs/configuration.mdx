...
# Choose the memory storage backend: local (default), qdrant-neo4j, or cognee
headroom proxy --memory --memory-backend cognee
```

The cognee backend requires `pip install "headroom-ai[cognee]"` and accepts
further tuning via `--memory-cognee-dataset`, `--memory-cognee-system-root`,
`--memory-cognee-data-root`, `--memory-cognee-search-type`,
`--memory-cognee-auto-cognify/--no-memory-cognee-auto-cognify`, and
`--memory-cognee-metadata-db` (each also reads the matching
`HEADROOM_COGNEE_*` env var; the flag wins when both are set). Note that
cognee's root-directory configuration is process-wide and immutable: the first
cognee backend initialized in a process fixes `system-root`/`data-root` for
every later instance in that process. See
[Memory](/docs/memory#cognee-backend-knowledge-graph).
...
| `HEADROOM_MEMORY_BACKEND` | Proxy memory storage backend: `local`, `qdrant-neo4j`, or `cognee`. `cognee` requires `pip install "headroom-ai[cognee]"`. Same as `--memory-backend`. See [Memory](/docs/memory#cognee-backend-knowledge-graph). | `local` |
| `HEADROOM_COGNEE_DATASET` | cognee dataset that holds all Headroom memories (cognee backend only). | `headroom_memories` |
| `HEADROOM_COGNEE_SYSTEM_ROOT` | Directory for cognee system state (databases, caches). Keeps Headroom's cognee state isolated from other cognee installs on the machine. | cognee's default |
| `HEADROOM_COGNEE_DATA_ROOT` | Directory for cognee data storage. | cognee's default |
| `HEADROOM_COGNEE_SEARCH_TYPE` | cognee `SearchType` name used for memory retrieval, e.g. `CHUNKS` (raw retrieval, no LLM synthesis) or `GRAPH_COMPLETION` (graph context; sent with `only_context=True` so no LLM answer is generated). | `CHUNKS` |
| `HEADROOM_COGNEE_AUTO_COGNIFY` | Whether `cognee.cognify()` runs after each save so new memories join the knowledge graph. cognify is LLM-bound, so it runs in the background and never blocks a save. `true`/`false`. | `true` |
| `HEADROOM_COGNEE_METADATA_DB` | Path to the SQLite file holding the cognee backend's durable memory registry and delete/update tombstones. Proxy workers that must share memories (and see each other's deletions) must point at the same file. Same as `--memory-cognee-metadata-db`. | `headroom_cognee_meta.db` under the cognee data root, else the system root, else `~/.headroom` |
