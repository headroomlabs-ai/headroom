...
The current DeepSeek model names are `deepseek-flash` and `deepseek-v4-pro`.
The retired `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` are still
accepted and billed at the Flash price; `deepseek-chat` and `deepseek-reasoner`
are deprecated compatibility aliases.

### Model comparison

All prices are USD per 1M tokens, at DeepSeek's **off-peak** tier. Peak hours —
Beijing 09:00-12:00 and 14:00-18:00, i.e. 01:00-04:00 and 06:00-10:00 UTC — cost
exactly twice these rates.

| Model | Input / Output (per 1M, off-peak) | Context | Max Output | Vision |
|---|---|---|---|---|
| `deepseek-flash` (V4.1-Flash) | $0.15 / $0.60 | 1M | 384K | yes |
| `deepseek-v4-pro` | $0.66 / $1.98 | 1M | 384K | no |
...
Select one with `/model deepseek-flash` or `/model deepseek-v4-pro`.
