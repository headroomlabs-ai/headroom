...
The flag means the same thing whether or not wrap starts the proxy:

```bash
# wrap starts the proxy and points it at DeepSeek
headroom wrap opencode --openai-api-url https://api.deepseek.com/v1

# reuse a proxy you started yourself — it must already target that upstream
headroom proxy --port 8787 --openai-api-url https://api.deepseek.com/v1
headroom wrap opencode --no-proxy --openai-api-url https://api.deepseek.com/v1
```

With `--no-proxy`, wrap never starts or re-points a proxy. It checks the one
listening on `--port` and exits with an error — before it rewrites
`opencode.json` — if nothing is listening, the listener is not a Headroom
proxy, or it forwards OpenAI-compatible traffic to a different upstream. Start
the proxy with the upstream you want, or drop `--no-proxy` and let wrap start
it.
...
| `HEADROOM_OPENCODE_EXCLUDE_HOSTS` | Comma-separated hosts (and their subdomains) the native plugin sends straight upstream instead of through the proxy, for example `opencode.ai,.corp.internal` |
...
The plugin routes every external HTTP(S) request through the proxy. To keep a provider or gateway off the proxy while leaving the plugin loaded, pass `excludeHosts` in the plugin options or set `HEADROOM_OPENCODE_EXCLUDE_HOSTS` before launching OpenCode. An entry matches the host and all of its subdomains; excluded requests are sent as-is, with no compression or telemetry.
...
**One provider rejects requests only while the plugin is loaded.**

Some gateways validate the client on the wire and reject the proxy hop. Exclude that host with `HEADROOM_OPENCODE_EXCLUDE_HOSTS=<host>` (or the `excludeHosts` plugin option); Headroom keeps routing every other provider.
