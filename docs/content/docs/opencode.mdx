...
| Fallback env | Sets `OPENAI_BASE_URL` and `ANTHROPIC_BASE_URL` as fallback base URLs |
...
| `OPENAI_BASE_URL` | Fallback OpenAI-compatible base URL pointing at the proxy |
| `ANTHROPIC_BASE_URL` | Fallback Anthropic base URL pointing at the proxy |
