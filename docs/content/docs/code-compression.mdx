...
| Tier 2 | Go, Rust, Java, C, C++, C#, PHP, Ruby | Function body compression |
...
Ruby is supported through parser-validated source edits. The compressor preserves
source order and untouched bytes, and fails open when Ruby syntax or statement
boundaries cannot be validated. Use `ruby` or `rb` for an explicit hint; ERB,
Rakefiles, and gemspecs remain unsupported formats.
