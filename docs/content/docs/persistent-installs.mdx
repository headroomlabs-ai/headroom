...
For the Python-native `persistent-docker` preset, if `docker` is a Podman wrapper
script rather than a symlink, set `HEADROOM_CONTAINER_RUNTIME=podman` in the
installer/runtime environment so it selects the Podman path.

On non-Windows hosts the Podman command combines `--userns=keep-id` with an
explicit `--user` using the invoking user's UID and GID. Both are needed on
Podman versions that honor the image's `USER root` even with `keep-id`: the
namespace preserves the host mapping, and the process user keeps newly created
files owned by the host user. The configured image is not changed.

Restart the deployment with the updated installer to use these flags. This
prevents new ownership mismatches; it does not repair files created by an older
container. Inspect and restore ownership of affected files separately before
restarting if the host user can no longer read them.
