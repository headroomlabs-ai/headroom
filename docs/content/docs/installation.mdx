...
The Vertex AI backend also requires `google-cloud-aiplatform>=1.38`, which ships
in the `[vertex]` extra (`pip install "headroom-ai[proxy,vertex]"`) rather than
`[proxy]`, since it adds roughly 175 MB and only that backend needs it. The
proxy refuses to start if the backend is selected without it. Vertex's native
passthrough routes need neither the extra nor the backend flag — see
