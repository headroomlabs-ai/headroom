...
How many items survive is derived from the content, not from a fixed target. The adaptive sizer finds the point of information saturation (Kneedle on unique bigram coverage) and keeps items up to it, so a repetitive array collapses further than a varied one of the same length. A `CompressionProfile` bias leans the result conservative or aggressive.
