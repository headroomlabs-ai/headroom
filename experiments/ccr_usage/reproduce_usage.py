...
def sse(message):
    ...
    for i, block in enumerate(message['content']):
        ...
        events += [('content_block_start', {'type': 'content_block_start', 'index': i, 'content_block': initial}),
                   ('content_block_delta', {'type': 'content_block_delta', 'index': i, 'delta': delta}),
                   ('content_block_stop', {'type': 'content_block_stop', 'index': i})]
    events += [('message_delta', {'type': 'message_delta', 'delta': {'stop_reason': message['stop_reason'], 'stop_sequence': None}, 'usage': {'output_tokens': message['usage']['output_tokens']}}),
...
def run():
    ...
    for key in list(os.environ):
        if key.startswith(('ANTHROPIC_', 'OPENAI_', 'HEADROOM_', 'LITELLM_')) or key.endswith(('_API_KEY', '_TOKEN')):
            os.environ.pop(key)
    os.environ.update(HF_HUB_OFFLINE='1', HF_HUB_DISABLE_TELEMETRY='1',
                      LITELLM_LOCAL_MODEL_COST_MAP='true', HEADROOM_BEACON='off')
    ...
    with tempfile.TemporaryDirectory(prefix='headroom-repro-') as temporary:
        ...
        class Provider(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                self.calls.append({'usage': usage(n), 'stream': body.get('stream', False),
                                   'retrieval_verified': n > 1})
                content = ([{'type': 'tool_use', 'id': f'retrieve_{n}', 'name': 'headroom_retrieve', 'input': {'hash': hash_key}}]
                           if n < self.target_calls else [{'type': 'text', 'text': 'synthetic final'}])
                message = dict(id=f'msg_{n}', type='message', role='assistant', model=body['model'],
                               content=content, stop_reason='tool_use' if n < self.target_calls else 'end_turn',
                               stop_sequence=None, usage=usage(n))
        ...
        config = ProxyConfig(anthropic_api_url=f'http://127.0.0.1:{provider.server_port}',
                             optimize=False, cache_enabled=False, rate_limit_enabled=False,
                             cost_tracking_enabled=False, log_requests=False,
                             ccr_inject_tool=True, ccr_handle_responses=True, ccr_context_tracking=False,
                             image_optimize=False, subscription_tracking_enabled=False,
                             memory_enabled=False, memory_inject_tools=False, memory_inject_context=False,
                             traffic_learning_enabled=False, read_lifecycle=False,
        ...
        try:
            ...
            for streaming in (False, True):
                for target in (1, 2, 3):
                    ...
                    body = dict(model='claude-haiku-4-5-20251001', max_tokens=100, stream=streaming,
                                messages=[{'role': 'user', 'content': f'Synthetic task; earlier output at <<ccr:{hash_key}>>'}],
                                tools=[create_ccr_tool_definition('anthropic')])
                    request = urllib.request.Request(f'http://127.0.0.1:{port}/v1/messages',
                              data=json.dumps(body).encode(), headers={'Content-Type': 'application/json',
                              'x-api-key': 'synthetic-local-only', 'anthropic-version': '2023-06-01',
                    ...
                    rows.append(dict(streaming=streaming, calls=target, upstream=Provider.calls,
                                     expected=expected, actual=actual, passed=actual == expected,
                                     usage_events=[json.loads(line[6:]) for line in raw.decode().splitlines()
                                                   if line.startswith('data: ') and json.loads(line[6:]).get('type') in ('message_start', 'message_delta')]
                                                   if 'text/event-stream' in content_type else []))
            args.output.write_text(json.dumps(rows, indent=2)+'\n')
            for row in rows:
                print(f"stream={row['streaming']} calls={row['calls']}: {'PASS' if row['passed'] else 'FAIL'} expected={row['expected']} actual={row['actual']}")
