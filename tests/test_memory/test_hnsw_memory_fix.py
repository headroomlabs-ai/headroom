...
class TestNoEmbeddingsDict:
    ...
    @pytest.mark.asyncio
    async def test_update_embedding_updates_hnsw_without_duplicate_cache(self):
        from headroom.memory.adapters.hnsw import HNSWVectorIndex
        from headroom.memory.ports import VectorFilter

        idx = HNSWVectorIndex(dimension=4, max_entries=10)
        memory = _make_memory()
        memory.embedding = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32)
        await idx.index(memory)
        replacement = np.array([0.0, 1.0, 0.0, 0.0], dtype=np.float32)

        assert await idx.update_embedding(memory.id, replacement) is True
        results = await idx.search(VectorFilter(query_vector=replacement, top_k=1))

        assert results[0].memory.id == memory.id
        assert np.allclose(results[0].memory.embedding, replacement)
        assert not hasattr(idx, "_embeddings")
