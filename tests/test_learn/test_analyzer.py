...
class TestPriorPatternsInjection:
    ...
    def test_digest_omits_section_when_context_file_ignored_for_learn(self, tmp_path):
        """A CLAUDE.md ignored for 'learn' (e.g. generated by cARL, #1150)
        must not be treated as a baseline the LLM should reconcile against."""
        project = _project_with_files(
            tmp_path, claude_md_text=f"# Project\n\n{_MARKER_BLOCK}\n", memory_md_text=None
        )
        (project.project_path / ".headroomignore").write_text("CLAUDE.md\n")
        digest = _build_digest(project, [])
        assert "Prior Learned Patterns" not in digest
