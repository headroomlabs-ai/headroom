...
def _bedrock_openai_caching_eligible(litellm_model: str) -> bool:
    ...
    for Bedrock models (litellm 1.96.2+), so it cannot gate this.
    ``litellm.utils.supports_prompt_caching`` is the API that knows (#3554).
    The *mapped* model (``bedrock/...``) must be checked: a client alias
    such as ``claude-sonnet-4-20250514`` alone resolves to False in
    litellm's registry. Never raises — an undecidable model simply gets
    no injected breakpoint.
    """
    ...
    try:
        ...
        logger.debug("supports_prompt_caching undecidable for %s: %s", litellm_model, e)
        return False
...
def _place_system_cache_control(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    ``cache_control`` anywhere, messages are returned unchanged. Never
    mutates the input: the proxy re-reads ``body["messages"]`` after the send.
    """
