...
SETTINGS: tuple[SettingField, ...] = (
    ...
    SettingField(
        "HEADROOM_ANTHROPIC_AUTO_CONTINUE",
        "anthropic_auto_continue",
        "Anthropic usage-limit auto-continue",
        "Timeouts",
        "bool",
        default=False,
        help="Keep a recognized Anthropic usage-limit SSE request open and retry once at reset.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_ANTHROPIC_AUTO_CONTINUE_MAX_WAIT_SECONDS",
        "anthropic_auto_continue_max_wait_seconds",
        "Anthropic auto-continue max wait (s)",
        "Timeouts",
        "float",
        default=18_300.0,
        minimum=1,
        maximum=86_400,
        help="Maximum reset delay held open; longer waits retain the original upstream status.",
        tier="advanced",
    ),
    SettingField(
