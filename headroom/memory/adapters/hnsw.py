...
class HNSWVectorIndex:
    ...
    async def update_embedding(self, memory_id: str, embedding: np.ndarray) -> bool:
        ...
        Re-adding an existing hnswlib label updates that point and repairs its
        graph connections, so search and returned memory data both observe the
        new vector without retaining a duplicate Python-side embedding cache.
    ...
    async def _update_embedding_internal(self, memory_id: str, embedding: np.ndarray) -> bool:
        ...
        self._index.add_items(
            embedding.reshape(1, -1),
            np.asarray([hnsw_id], dtype=np.int64),
        )
        return True
