"""CodeBuddy memory sync adapter.

Reads/writes CodeBuddy's native memory format:
    ~/.codebuddy/projects/<sanitized-path>/memory/
...
def encode_codebuddy_project_path(project_path: Path | str) -> str:
    """Encode a project path the way CodeBuddy names project directories.
            ...
            memories.append(
                AgentMemory(
                    ...
                    metadata={
                        ...
                        "source_agent": fm.get("source_agent", "codebuddy"),
                    },
                )
            )
...
def get_codebuddy_memory_dir(project_path: Path | None = None) -> Path:
    """Get the CodeBuddy memory directory for a project.

    CodeBuddy stores per-project memory at:
        ~/.codebuddy/projects/-<sanitized-path>/memory/
    """
    project = project_path or Path.cwd()
    sanitized = encode_codebuddy_project_path(project)
    return Path.home() / ".codebuddy" / "projects" / sanitized / "memory"
