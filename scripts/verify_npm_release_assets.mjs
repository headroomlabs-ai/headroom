...
if (!assetsDirArg || !version) {
	console.error(
		"Usage: node scripts/verify_npm_release_assets.mjs <assets-dir> <version>",
	);
	process.exit(2);
}
...
function extractJsonFromTarball(tarballPath, packageJsonPath) {
	...
	try {
		...
		return JSON.parse(
			readFileSync(path.join(workdir, packageJsonPath), "utf8"),
		);
	} finally {
...
function assertNoFileDependencies(pkg) {
	for (const field of [
		...
		for (const [name, spec] of Object.entries(pkg[field] || {})) {
			if (
				typeof spec === "string" &&
				(spec.startsWith("file:") || spec.includes("release-assets"))
			) {
				throw new Error(
...
for (const expected of packages) {
	...
	if (pkg.version !== version) {
		throw new Error(
			`${expected.tarball} version mismatch: expected ${version}, got ${pkg.version}`,
		);
	}
	...
	if (expected.extensions) {
		if (
			JSON.stringify(pkg.pi?.extensions) !== JSON.stringify(expected.extensions)
		) {
			throw new Error(
	...
	if (expected.name === "headroom-openclaw") {
		...
		if (distPkg.name !== expected.name) {
			throw new Error(
				`${expected.tarball} dist package name mismatch: expected ${expected.name}, got ${distPkg.name}`,
			);
		}
		if (distPkg.version !== version) {
			throw new Error(
				`${expected.tarball} dist package version mismatch: expected ${version}, got ${distPkg.version}`,
			);
		}
		...
		for (const [name, spec] of Object.entries(expected.dependencies || {})) {
			...
			if (actual !== spec) {
				throw new Error(
					`${distPkg.name} dist dependency ${name} mismatch: expected ${spec}, got ${actual}`,
				);
			}
		}
	}
...
try {
	for (const expected of packages) {
		copyFileSync(
			tarballPaths.get(expected.name),
			path.join(installDir, expected.tarball),
		);
	}
	const result = runNpm(
		[
